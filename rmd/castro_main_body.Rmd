...
dffinal <- here::here("./data/castro_iso_fauna.csv") %>%
...
```{r castro-archbot-assemblage, echo=FALSE, dpi=600, fig.show="hold", out.width='0.98\\textwidth', fig.env='figure*', fig.cap= '(ref:archbot-caption)'}
...
```{r carb-nitro-iso-plot, dpi=600, echo=FALSE, message = FALSE, error = FALSE, warning = FALSE, out.width='0.98\\textwidth', fig.height=8, fig.env='figure*', fig.cap= 'Plot showing isotope values of faunal bone collagen.'}


theme1<-theme(panel.background = element_blank(),legend.key.size = unit(0.6, 'cm'),legend.text = element_text(size=6),text = element_text(size=7),legend.title=element_text(size=7),axis.title.x = element_text(size=5), axis.title.y = element_text(size=5), panel.border=element_rect(fill=NA),panel.grid.major = element_blank(),panel.grid.minor = element_blank(),strip.background=element_blank(),axis.text.x=element_text(colour="black"),axis.text.y=element_text(colour="black"),axis.ticks=element_line(colour="black"),plot.margin=unit(c(1,1,1,1),"line"))

carb_nitro_iso_plot <- ggplot(dffinal, aes(x = dC, y = dN, shape = factor(Species), colour = factor(Species))) + geom_point(size = 2, alpha = 0.5) + scale_color_aaas() + scale_shape_manual(values = c(15,16,17,18,19,15,16,17,18,19)) +
labs(x = expression(paste(delta^{13}, "C (\u2030 vs. VPDB)")), y = expression(paste(delta^{15}, "N (\u2030 vs. Air)")), shape = 'Species', colour = 'Species') + coord_fixed(0.5) + scale_y_continuous(breaks=seq(2.5,16,1)) + scale_x_reverse(breaks=seq(-22,-14,1)) + theme1
