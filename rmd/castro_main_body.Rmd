...
df_iso_fauna_summ %>%
  ...
  kable(caption = "Statistical summary of mean and standard deviation of isotope composition of the fauna.", digits = 2, booktabs = T,linesep = "", align = "ccllllll", format = "latex", escape = FALSE,table.env='table*') %>%
add_header_above(c(" " = 2, "$\\delta^{13}C (\\text{\\textperthousand})$" = 2, "$\\delta^{15}N (\\text{\\textperthousand})$" = 2, "$\\delta^{34}S (\\text{\\textperthousand})$" = 2))
...
All fifty faunal samples with the exception of rabbits (in case of $\delta ^{15}N$), demonstrate (Table \ref{tab:table4}) stable isotope values within the range expected for a C~3~ temperate ecosystem. 
 Sheep show mean $\delta ^{13}C$ values of `r iso_meandC_sheep` ± `r iso_stddevC_sheep`\permil and mean $\delta ^{15}N$ values of `r iso_meandN_sheep` ± `r iso_stddevN_sheep`\permil, while goats exhibit mean $\delta ^{13}C$ values of `r iso_meandC_goat` ± `r iso_stddevC_goat`\permil and mean $\delta ^{15}N$ values of `r iso_meandN_goat` ± `r iso_stddevN_goat`\permil. Goat mean $\delta ^{13}C$ values are not significantly different than that of sheep (t-statistic: `r sheep_goat_dC_ttest_statistic`, degrees of freedom: `r sheep_goat_dC_ttest_df`, *p*-value: `r sheep_goat_dC_ttest_p`). The $\delta ^{13}C$ values of ovicaprines are indicative of feeding in forested patches of land. Feeding in forested areas causes the $\delta ^{13}C$ values to increase due to canopy effect [@bonafini_etal13]. Sheep also exhibit significantly higher $\delta ^{15}N$ values than goats (t-statistic: `r sheep_goat_dN_ttest_statistic`, degrees of freedom: `r sheep_goat_dN_ttest_df`, *p*-value: `r sheep_goat_dN_ttest_p`) which demonstrates that sheep were fed on food sourced from manured vegetation (Fig. \ref{fig:auna-carbnitro-iso-plot}. Sheep are superior to goats both in terms of secondary products and ease of management [@davis07; @rutter02]. Owing to the more attached economic interests with sheep, it is natural to give food sourced from cultivated crops.
