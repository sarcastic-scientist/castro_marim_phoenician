...
knitr::include_graphics(
  'C:/Users/rosha/Documents/test01/Castro Marim Map.jpg',
...
dffinal <- here::here("castrokable.csv") %>%
...
dffinal %>%
  ...
  kable(caption = "Isotope Data of Castro Marim", digits = 2, booktabs = T, format = "latex", escape = FALSE, col.names=c("ID","Phase","Species","Element", "$\\delta^{13}C (\\text{\\textperthousand})$","$\\delta^{15}N (\\text{\\textperthousand})$","$\\delta^{34}S (\\text{\\textperthousand})$")) %>%
...
setwd('C:/Users/rosha/Documents/')
#Importing the data in ASCII format 
Dfraw = read.table(file = 'Castrofauna.txt', header = TRUE)
#Ordering the sample occurrence according to species
Dfmain = Dfraw[order(Dfraw$Species), ]
...
compplot<-ggplot(Dfmain, aes(x = dC, y = dN, shape = factor(Species), colour = factor(Species))) + geom_point(size = 4, alpha = 0.65) + scale_color_aaas() + scale_shape_manual(values = c(15,16,17,18,19,15,16,17,18,19)) +
