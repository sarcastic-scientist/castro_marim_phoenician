...
dffinal %>%
  select(Sample,Phase, Species, Element) %>%
  kable(caption = "Selected faunal samples from all phases of settlement.", digits = 2, align="lccr", booktabs = T, format = "latex", escape = FALSE,table.env='table*', col.names=c("ID","Phase","Species","Element")) %>%
  kable_styling(latex_options = c("basic", "hold_position","repeat_header","scale_down"), full_width = TRUE) %>%
...
dffinal %>%
  select(Sample, Species, Element, dC, dN, dS) %>%
  kable(caption = "Carbon, nitrogen, and sulphur isotope composition of the fauna.", digits = 2, align="lccrrc", booktabs = T, format = "latex", escape = FALSE,table.env='table*', col.names=c("ID","Species","Element", "$\\delta^{13}C (\\text{\\textperthousand})$","$\\delta^{15}N (\\text{\\textperthousand})$","$\\delta^{34}S (\\text{\\textperthousand})$")) %>%
  kable_styling(latex_options = c("basic", "hold_position","repeat_header","scale_down"), full_width = TRUE) %>%
